...
!!! note "大白话"
    这个公式就是按输入大小、核大小、补边和步幅，算出卷积后还剩多少格。

- 其中，$\text{in}$ 是输入尺寸，$k$ 是核大小，$p$ 是填充，$s$ 是步幅。该公式分别适用于高度和宽度。

!!! note "大白话"
    高度和宽度各套一次同样的公式，就能得到输出特征图的二维尺寸。

- 一个神经元的**感受野（receptive field）**是原始输入中能够影响其取值的区域。
    - 前面的层感受野较小（能看到边缘等局部模式）。

        !!! note "大白话"
            浅层神经元只看原图的一小块，适合发现边缘、线条这类局部信号。

    - 更深的层感受野较大（能看到物体部件等较大结构）。

        !!! note "大白话"
            深层神经元把前面许多小区域的信息合起来，所以能看到眼睛、轮子等更大的部件。

!!! note "大白话"
    感受野就是一个神经元“能看到原图多大一块”；越深通常看得越宽，从边缘逐渐到部件。

- 感受野随每一层增长：每个卷积层大致增长 $k - 1$ 个像素（使用步幅或膨胀时增长更多）。

!!! note "大白话"
    层层叠加会把局部视野串起来，步幅和膨胀会让视野扩大得更快。
...
!!! note "大白话"
    BatchNorm 把每批数据的数值范围调稳，训练不容易忽大忽小，因此可以用更积极的学习率。

- **Dropout**（第 06 章）会在训练期间随机将神经元置零。

!!! note "大白话"
    Dropout 训练时随机关掉一部分神经元，逼网络别过度依赖某几个固定通路。

- 在 CNN 中，**空间 Dropout（spatial dropout，Dropout2D）**会丢弃整个特征图通道，而非单个像素。由于特征图中相邻像素高度相关，这种方式更有效。

!!! note "大白话"
    图像相邻像素很像，逐点丢弃作用有限；整条通道一起丢，更能迫使模型学不同的特征。
...
!!! note "大白话"
    数据增强把同一张图随机翻转、裁剪或变色，让模型学“是什么”而不是死记某几个像素。
...
!!! note "大白话"
    AlexNet 把 ReLU、Dropout、数据增强和 GPU 训练组合起来，证明更深更大的 CNN 能在大规模图像集上胜出。
...
!!! note "大白话"
    小卷积核多堆几次能看到同样大的范围，却参数更少、非线性更多，所以 VGG 选择简单地重复 3x3。
...
!!! note "大白话"
    Inception 同时用多个尺度看图，再把结果拼起来；1x1 瓶颈先压通道，避免大卷积太贵。
...
!!! note "大白话"
    DenseNet 让每层都能直接看到前面所有层的结果，特征可以复用，梯度也更容易传回去。
...
!!! note "大白话"
    MobileNet 先各通道单独看空间，再用 1x1 混合通道，把昂贵卷积分成两步省计算。
...
!!! note "大白话"
    标准卷积同时做空间和通道混合，深度可分离卷积拆开后乘法数大幅减少，3x3 时大约省到九分之一。
...
!!! note "大白话"
    MobileNet-V2 在宽空间里做主要计算，输入输出保持窄，正好和传统 ResNet 的瓶颈方向相反。
...
!!! note "大白话"
    EfficientNet 用一个旋钮同时增加层数、通道数和输入分辨率，避免只放大某一个维度造成失衡。
...
!!! note "大白话"
    这个比例让模型每次变大时计算量大致翻倍，B0 到 B7 就是同一设计逐级放大。
...
!!! note "大白话"
    分组卷积先把通道分组省计算，再洗牌让不同组重新交换信息，避免各组彼此隔离。
...
!!! note "大白话"
    从特征倒推图片，能看出浅层保留细节，深层更重视“是什么”而不是精确像素。
...
!!! note "大白话"
    Deep Dream 放大模型喜欢的模式，风格迁移则把一张图的内容和另一张图的纹理风格拼到一起。
