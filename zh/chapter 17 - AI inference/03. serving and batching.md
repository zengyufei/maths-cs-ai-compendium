...
!!! note "大白话"
    模型能回答一个人，不代表能高效服务一万人。在线推理的难点是把不同长度、不同优先级的请求塞进 GPU，同时不让用户等得太久。
...
!!! note "大白话"
    Prefill 能把已知提示词一起算，GPU 很忙；decode 必须等上一个 token 出来后才能继续，而且每一步都得读大量权重。LLM 服务最难提速的通常正是这个逐 token 的阶段。
...
!!! note "大白话"
    Prefill 像先读完问题并做准备，影响用户多久看到第一个字；decode 像逐字输出答案，影响文字滚动得快不快。两段的瓶颈不同，不能用同一种批处理策略一把抓。
...
!!! note "大白话"
    静态批处理把一组人绑在一起，所有人都要等最慢的那个；长度不齐还得给短请求补大量空白。GPU 看似在工作，很多计算其实没有服务任何真实 token。
...
!!! note "大白话"
    连续批处理的关键是“完成即离开，排队即补位”。它不要求每个请求同长，所以面对真实世界里长短不一的输出，比静态 batch 更贴近 GPU 的工作方式。

- **影响**：连续批处理相较静态批处理通常提升 2 至 10 倍吞吐量，不改变模型质量，也几乎不增加延迟。

!!! note "大白话"
    连续批处理把“一个请求从头到尾”改成“每轮给所有活跃请求各生成一个 token”。有人结束就立刻换新人上来，GPU 槽位始终尽量坐满。
...
!!! note "大白话"
    固定大小页面让显存像拼积木一样可复用，避免长短不同请求留下无法利用的碎片。共同前缀还能共享同一批页面，直到两条请求真的走向不同内容。
...
!!! note "大白话"
    PagedAttention 把 KV-cache 像操作系统虚拟内存一样分成固定页面，用多少分多少，不要求每个请求占一整段连续显存。这样长度各异的大量请求能更紧凑地共存。
...
!!! note "大白话"
    FCFS 最容易实现但可能被超长请求堵住，SJF 平均更快但会亏待长请求，抢占能让紧急请求插队。线上调度没有无代价的公平，只能按产品承诺选择规则。

- **Token 预算**：限制活跃 batch 中 token 总数，避免少量长请求垄断 GPU 内存并使新请求饥饿。

!!! note "大白话"
    调度不只是排先后，还要管公平和内存。只追求平均吞吐量会让长任务堵住所有人，只追求短任务又可能让长任务永远轮不到，因此要设优先级、抢占和 token 上限。
...
!!! note "大白话"
    解耦服务把两类 GPU 都用在擅长的阶段，但 KV-cache 必须从 prefill 节点传到 decode 节点。这条传输链路若慢或拥堵，反而会成为新的 TTFT 瓶颈。

- 这是 **Mooncake**，Moonshot AI 的架构，也被多个 LLM 服务团队探索。收益是每种 GPU 类型都匹配工作负载特征，提高整体利用率。

!!! note "大白话"
    Prefill 要算力，decode 要显存带宽，把它们放在同一种 GPU 上会互相抢资源。拆成两类节点后，各自选最擅长的硬件，代价是还要高效传输初始 KV-cache。
...
!!! note "大白话"
    多模型服务可以按价格和难度分层，但每多加载一个模型就少一块 KV-cache 空间。模型数、并发量和缓存预算必须一起规划，不能只看权重能否勉强装下。
...
!!! note "大白话"
    LoRA 服务不必为每个客户复制一整套基础模型，只要共用底座、按请求挂上很小的适配器。真正难的是让不同 adapter 的请求仍能一起批处理，不把 GPU 拆成一堆小任务。
...
!!! note "大白话"
    受约束生成不是生成后再检查 JSON 对不对，而是在每一步提前禁掉不可能合法的 token。它把格式问题从“出错后重试”变成“根本不允许走错路”。
...
!!! note "大白话"
    级联路由先让小模型回答，只有不确定时才升级。关键风险是置信度不等于正确性，路由阈值必须在真实业务数据上评估，否则省下成本会变成质量事故。
...
!!! note "大白话"
    请求路由像分诊：简单问题交给便宜小模型，难题才升级到大模型。路由判断做得准，绝大多数请求的成本会下降，同时用户仍能在复杂问题上得到高质量回答。
...
!!! note "大白话"
    对话产品要优先看首字和逐字速度，离线批处理才把总 token/s 拉到最大。一个高吞吐设置未必适合交互式用户，指标必须按产品场景定。
...
!!! note "大白话"
    TTFT 和 TPOT 看用户体验，吞吐量和每 token 成本看生意能否成立。批太大可能让用户变慢，批太小又浪费 GPU，服务系统本质上是在这些目标间不断折中。
